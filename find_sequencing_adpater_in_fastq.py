...
def main():
	...
	parser = argparse.ArgumentParser(description = "Look for the first 7nt of the sequencing adapters\n"
...
def adapterPositions(forward, reverse):
	...
	while True:
		try:
			...
			pos_f = seq_f.find('AGATCGG')
			pos_r = seq_r.find('AGATCGG')

			newseq_f = seq_f.split("AGATCGG")[0]+"--AGATCGG--"+seq_f.split("AGATCGG")[-1]
			newseq_r = seq_r.split("AGATCGG")[0]+"--AGATCGG--"+seq_r.split("AGATCGG")[-1]
